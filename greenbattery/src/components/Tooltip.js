...
const Tooltip = ({
  ...
  const followPoints = React.useCallback(
    (e) => {
      ...
      d3.select(ref.current)
        .selectAll(`#${parentId} .tooltipLinePoint`)
  ...
  React.useEffect(() => {
    d3.select(anchorEl)
      .on("mouseout.tooltip", () => {
        ...
        d3.selectAll(`#${parentId} .tooltip`).attr("opacity", 0);
        console.log(anchorEl.id);
      })
      .on("mouseover.tooltip", () => {
        d3.select(ref.current).attr("opacity", 1);
        d3.selectAll(`#${parentId} .tooltip`).attr("opacity", 1);
      })
